...
function shellExec(env) {
	...
	while ((thread) || (curLine!='')) {
		...
		while (usrALIAS[args[0]]) {
			if (aref[args[0]]) {
				kernel.krnlFOut(env.stderr,'error: circular aliases.');
		...
		if (cmd.charAt(0)=='#') {
			...
			kernel.krnlFOut(env.stderr,'"'+cmd+'" is not a command.');
		}
			...
			for (var pi=0; pi<cmdpath.length; pi++) {
				...
				if ((typeof cmdf=='object') && (cmdf.lines[0]) && ((cmdf.kind=='b') || (cmdf.kind=='f')) && (cmdf.lines[0].indexOf('#!/dev/js')==0)) {
					...
					if (cmdf.callback) {
						...
						kernel.krnlFOut(env.stderr,'error: can\'t execute binary!');
						if (curLine) continue
					}
				}
		...
		if (args.length>1) {
			if ((args[0]=='sh') && (child.stdin==null)) {
				...
				if ((typeof srcf=='object') && (srcf.kind=='f')) {
					...
					kernel.krnlFOut(env.stderr,vfsGetPath(args[1], env.cwd)+': permission denied.');
					var cl=new Array();
...
function shellWait(thread,env, forceExit) {
	...
	if (forceExit) {
		env.status='';
		env.bin='';
		if (env.loginShell) kernel.krnlTTY();
	}
	else if ((thread==false) || (env.status)) {
		if (env.loginShell) {
			env.stdin=null;
			kernel.krnlTTY(env,'shellExec')
		}
		else {
			env.status='wait';
			env.bin='shellExec'
...
function shellWriteOut(env,fn,pipe,append) {
	...
	if (f1) {
		...
		if (f1<0) {
			...
			kernel.krnlFOut(env.stderr,'redirect - can\'t write, file is directory: '+path);
		}
		else {
		...
		if (f==0) {
			kernel.krnlFOut(env.stderr,'redirect - illegal file path: '+path);
		}
		else if (f<0) {
			kernel.krnlFOut(env.stderr,'redirect - permission denied: '+path);
		}
...
function shellCmdSet(env,args) {
	...
	for (var i=a; i<args.length; i++) {
		if (args[i]=='=') {
			assign=true;
			if (keys.length==0) {
				if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' [<varname> [<varname] [= <value> [<value>]]]\n - no varnames');
				return
			}
		}
		else if (assign) {
	...
	for (var k=0; k<keys.length; k++) {
		...
		if (shellTestName(ks)==false) {
			...
			if (verbous) kernel.krnlFOut(env.stderr,'error: "'+ks+'" is a reserved name!');
		}
		else {
			usrVAR[ks]=val;
			if (verbous) kernel.krnlFOut(env.stderr,'var "'+ks+'" set to "'+val+'".')
		}
	}
}
...
function shellCmdUnset(env,args) {
	...
	if (args.length<a+1) {
		if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' <varname> - no varname');
		return
	};
	var k=args[a];
	if (shellTestName(k)==false) {
		if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' <varname> - name must be in [A-Za-z0-9_]');
		return
	};
	if ((k=='PID') || (k=='PATH') || (k=='USER') || (k=='HOME') || (k=='VERSION') || (k=='HOST') || (k=='UID') || (k=='GID')) {
		if (verbous) kernel.krnlFOut(env.stderr,'error: "'+k+'" is a reserved name!');
		return
	}
	else {
		delete(usrVAR[k]);
		if (verbous) kernel.krnlFOut(env.stderr,'unset var "'+k+'".');
	}
}
...
function shellCmdAlias(env,args) {
	...
	if (args.length<a+2) {
		if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' <name> <value>');
		return
	};
	var k=args[a];
	if (shellTestName(k)==false) {
		if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' <name> <value> - name must be in [A-Za-z0-9_] - (use quotes?)');
		return
	};
	var va=new Array();
	...
	var v=va.join(' ');
	usrALIAS[k]=v;
	if (verbous) kernel.krnlFOut(env.stderr,'alias "'+k+'" set to "'+v+'".');
}
...
function shellCmdUnalias(env,args) {
	...
	if (args.length<a+1) {
		if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' <name> - (use quotes?)');
		return
	};
	var k=args[a];
	if (shellTestName(k)==false) {
		if (verbous) kernel.krnlFOut(env.stderr,'usage: '+args[0]+' <name> - name must be in [A-Za-z0-9_]');
		return
	};
	delete(usrALIAS[k]);
	if (verbous) kernel.krnlFOut(env.stderr,'deleted alias "'+k+'".')
}
...
function shellCmdCd(env,args) {
	...
	var d=vfsGetDir(p);
	if (d<0) {
		kernel.krnlFOut(env.stderr,p+': permission denied.')
	}
	else if (d==0) {
		kernel.krnlFOut(env.stderr,p+': no such directory.')
	}
...
function shellCmdPwd(env) {
	kernel.krnlFOut(env.stdout,env.cwd);
}
